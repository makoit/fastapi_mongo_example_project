...
async def get_all_students(limit: int = 50, skip: int = 0):
    ...
    students = await db_student_access.get_all_students(limit, skip)
...
async def get_student_by_id(id: str):
    ...
    """
    student = await db_student_access.get_student(id)
    return student
...
async def delete_workshop_by_id(id: str):
    ...
    if delete_response:
        ...
        raise HTTPException(status_code=status.HTTP_304_NOT_MODIFIED, detail="Student not deleted")
...
async def update_student(id: str, student_data: UpdateStudentModel):
    ...
    """
    student = await db_student_access.update_student_data(id, student_data.dict())

    if student:
        return student
    else:
        raise HTTPException(status_code=status.HTTP_304_NOT_MODIFIED, detail="Student not modified")
...
async def add_student(student: StudentModel):
    ...
    new_student = await db_student_access.add_new_student(student.dict())

    if new_student:
        return new_student
    else:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Student could not created")
